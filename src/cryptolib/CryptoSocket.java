...
public class CryptoSocket implements CryptoSocketInterface {
		private Channel channel = Channel.WLAN;
		...
		*/
			...
			if (this.channel == Channel.WLAN){
				...
				while(this.running){
					while (true){
						...
						if (!this.id.equals("") && !this.id.equals(socket.getRemoteSocketAddress().toString())){
							socket.close();
							continue;
						}
		...
		public boolean connect() throws IllegalArgumentException, IOException, SocketTimeoutException {
			if (this.channel == Channel.WLAN){
				...
				if (!this.id.equals("")){
					String tmp = new StringBuilder(this.id).reverse().toString();
